...
func parseExpr(p *Parser, bp BINDING_POWER) ast.Node {
	...
	if !exists {
		...
		report.Add(p.FilePath, p.currentToken().Start.Line, p.currentToken().End.Line, p.currentToken().Start.Column, p.currentToken().End.Column, msg, report.SYNTAX_ERROR)
	}
...
func parsePostfixExpr(p *Parser, left ast.Node, bp BINDING_POWER) ast.Node {
	...
	if _, ok := left.(ast.IdentifierExpr); !ok {
		report.Add(p.FilePath, left.StartPos().Line, left.EndPos().Line, left.StartPos().Column, left.EndPos().Column, "only identifiers can be incremented or decremented", report.SYNTAX_ERROR)
	}
	operator := p.advance()
...
func parseUnaryExpr(p *Parser) ast.Node {
	...
	case lexer.MINUS_TOKEN, lexer.NOT_TOKEN:
		break
	default:
		report.Add(p.FilePath, operator.Start.Line, operator.End.Line, operator.Start.Column, operator.End.Column, fmt.Sprintf("invalid unary operator '%s'", operator.Value), report.SYNTAX_ERROR)
	}
