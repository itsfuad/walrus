...
func checkImplStmt(implStmt ast.ImplStmt, env *TypeEnvironment) Tc {
	...
	if env.scopeType != GLOBAL_SCOPE {
		report.Add(env.filePath, implStmt.Start.Line, implStmt.End.Line, implStmt.Start.Column, implStmt.End.Column, "implement statement must be at global scope", report.CRITICAL_ERROR)
		return NewVoid()
	}
	...
	if err != nil {
		report.Add(env.filePath, implStmt.Start.Line, implStmt.End.Line, implStmt.Start.Column, implStmt.End.Column, err.Error(), report.CRITICAL_ERROR)
	}

	// type must be a struct
	implForType, ok := structValue.(Struct)
	if !ok {
		report.Add(env.filePath, implStmt.Start.Line, implStmt.End.Line, implStmt.Start.Column, implStmt.End.Column, "only structs can be implemented", report.CRITICAL_ERROR)
	}
	...
	for _, method := range implStmt.Methods {
		...
		if _, ok := implForType.StructScope.variables[name]; ok {
			report.Add(env.filePath, method.Start.Line, method.End.Line, method.Start.Column, method.End.Column, fmt.Sprintf("'%s' is already defined in struct '%s'", name, implForType.StructName), report.CRITICAL_ERROR)
		}
