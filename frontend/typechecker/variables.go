...
func checkVariableAssignment(node ast.VarAssignmentExpr, env *TypeEnvironment) Tc {
	...
	if err := checkLValue(Assignee, env); err != nil {
		report.Add(env.filePath, Assignee.StartPos().Line, Assignee.EndPos().Line, Assignee.StartPos().Column, Assignee.EndPos().Column, fmt.Sprintf("cannot assign to %s", err.Error()), report.CRITICAL_ERROR)
	}
	...
	if err != nil {
		report.Add(env.filePath, valueToAssign.StartPos().Line, valueToAssign.EndPos().Line, valueToAssign.StartPos().Column, valueToAssign.EndPos().Column, err.Error(), report.NORMAL_ERROR)
	}
...
func checkVariableDeclaration(node ast.VarDeclStmt, env *TypeEnvironment) Tc {
	...
	for _, varToDecl := range varsToDecl {
		...
		if varToDecl.Value != nil && varToDecl.ExplicitType != nil {
			...
			if err != nil {
				report.Add(env.filePath, varToDecl.Value.StartPos().Line, varToDecl.Value.EndPos().Line, varToDecl.Value.StartPos().Column, varToDecl.Value.EndPos().Column, fmt.Sprintf("error declaring variable '%s'. %s", varToDecl.Identifier.Name, err.Error()), report.NORMAL_ERROR)
			}
		}

		err := env.declareVar(varToDecl.Identifier.Name, expectedTypeInterface, node.IsConst, false)
		if err != nil {
			report.Add(env.filePath, varToDecl.Identifier.StartPos().Line, varToDecl.Identifier.EndPos().Line, varToDecl.Identifier.StartPos().Column, varToDecl.Identifier.EndPos().Column, err.Error(), report.CRITICAL_ERROR)
		}
