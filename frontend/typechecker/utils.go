...
func evalDefaultType(defaultType ast.DataType, env *TypeEnvironment) Tc {
	...
	if err != nil || val == nil {
		report.Add(env.filePath, defaultType.StartPos().Line, defaultType.EndPos().Line, defaultType.StartPos().Column, defaultType.EndPos().Column, err.Error(), report.CRITICAL_ERROR)
	}
	return val
}
...
func evalUD(analyzedUD ast.UserDefinedType, env *TypeEnvironment) Tc {
	...
	if err != nil || val == nil {
		report.Add(env.filePath, analyzedUD.StartPos().Line, analyzedUD.EndPos().Line, analyzedUD.StartPos().Column, analyzedUD.EndPos().Column, err.Error(), report.CRITICAL_ERROR)
	}
	return val
}
	...
	for _, param := range analyzedFunctionType.Parameters {
		...
		if utils.Some(params, func(p FnParam) bool {
			...
			report.Add(env.filePath, param.Identifier.Start.Line, param.Identifier.End.Line, param.Identifier.Start.Column, param.Identifier.End.Column,
				fmt.Sprintf("parameter '%s' is already defined", param.Identifier.Name), report.CRITICAL_ERROR)
		}
...
func evalMap(analyzedMap ast.MapType, env *TypeEnvironment) Tc {
	if analyzedMap.Map.Name == "map" {
		...
		if err != nil {
			report.Add(env.filePath, analyzedMap.StartPos().Line, analyzedMap.EndPos().Line, analyzedMap.StartPos().Column, analyzedMap.EndPos().Column, err.Error(), report.NORMAL_ERROR)
		}
		...
		report.Add(env.filePath, analyzedMap.StartPos().Line, analyzedMap.EndPos().Line, analyzedMap.StartPos().Column, analyzedMap.EndPos().Column, fmt.Sprintf("'%s' is not a map", analyzedMap.Map.Name), report.CRITICAL_ERROR)
