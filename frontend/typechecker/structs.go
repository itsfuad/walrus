...
func checkStructLiteral(structLit ast.StructLiteral, env *TypeEnvironment) Tc {
	...
	if err != nil {
		report.Add(env.filePath, sName.StartPos().Line, sName.EndPos().Line, sName.StartPos().Column, sName.EndPos().Column, err.Error(), report.NORMAL_ERROR)
	}

	structType, ok := Type.(Struct)

	if !ok {
		report.Add(env.filePath, sName.StartPos().Line, sName.EndPos().Line, sName.StartPos().Column, sName.EndPos().Column, fmt.Sprintf("'%s' is not a struct", sName.Name), report.CRITICAL_ERROR)
	}
...
func composeErrors(structLit ast.StructLiteral, missingProps []string, env *TypeEnvironment) {
	...
	report.Add(env.filePath, structLit.StartPos().Line, structLit.EndPos().Line, structLit.StartPos().Column, structLit.EndPos().Column, errMsg, report.NORMAL_ERROR)
}
...
func checkPropsType(structType Struct, structLit ast.StructLiteral, env *TypeEnvironment) {
	for _, structProp := range structLit.Properties {
		...
		if _, ok := structType.StructScope.variables[structProp.Prop.Name]; !ok {
			report.Add(env.filePath, structProp.Prop.Start.Line, structProp.Prop.End.Line, structProp.Prop.Start.Column, structProp.Prop.End.Column, fmt.Sprintf("property '%s' is not defined on struct '%s'", structProp.Prop.Name, structLit.Identifier.Name), report.CRITICAL_ERROR)
		}
		...
		if err != nil {
			report.Add(env.filePath, structProp.Prop.StartPos().Column, structProp.Value.EndPos().Line, structProp.Prop.StartPos().Column, structProp.Value.EndPos().Column, err.Error(), report.NORMAL_ERROR)
		}
	}
}
...
func checkPropertyAccess(expr ast.StructPropertyAccessExpr, env *TypeEnvironment) Tc {
	...
	case Interface:
		...
		report.Add(env.filePath, prop.Start.Line, prop.End.Line, prop.Start.Column, prop.End.Column, fmt.Sprintf("interface '%s' does not have a method '%s'", t.InterfaceName, prop.Name), report.CRITICAL_ERROR)
	}
	...
	if property, ok := structEnv.variables[prop.Name]; ok {
		...
		case StructProperty:
			...
			isPrivate = t.IsPrivate
		default:
			report.Add(env.filePath, prop.Start.Line, prop.End.Line, prop.Start.Column, prop.End.Column, fmt.Sprintf("'%s' is not a %s", prop.Name, propType), report.CRITICAL_ERROR)
		}

		if isPrivate {
			//check the scope we are in
			if !env.isInStructScope() {
				report.Add(env.filePath, prop.Start.Line, prop.End.Line, prop.Start.Column, prop.End.Column, fmt.Sprintf("cannot access private property '%s' from outside of the struct's scope", prop.Name), report.NORMAL_ERROR)
			}
		}
		return property
	}

	report.Add(env.filePath, prop.Start.Line, prop.End.Line, prop.Start.Column, prop.End.Column, fmt.Sprintf("'%s' does not exist on type '%s'", prop.Name, objName), report.CRITICAL_ERROR)
...
func checkStructTypeDecl(name string, structType ast.StructType, env *TypeEnvironment) Struct {
	...
	for _, propval := range structType.Properties {
		...
		if err != nil {
			report.Add(env.filePath, propval.PropType.StartPos().Line, propval.PropType.EndPos().Line, propval.PropType.StartPos().Column, propval.PropType.EndPos().Column, fmt.Sprintf("error declaring property '%s': %s", propval.Prop.Name, err.Error()), report.CRITICAL_ERROR)
		}
	}
	...
	if err != nil {
		report.Add(env.filePath, structType.StartPos().Line, structType.EndPos().Line, structType.StartPos().Column, structType.EndPos().Column, err.Error(), report.CRITICAL_ERROR)
	}
