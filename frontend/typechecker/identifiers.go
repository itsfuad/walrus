...
func checkIdentifier(node ast.IdentifierExpr, env *TypeEnvironment) Tc {
	...
	if isTypeDefined(name) && (name != "null" && name != "void") {
		report.Add(env.filePath, node.StartPos().Line, node.EndPos().Line, node.StartPos().Column, node.EndPos().Column, "cannot use type as value", report.CRITICAL_ERROR)
	}

	//find the declaredEnv where the variable was declared
	declaredEnv, err := env.resolveVar(name)
	if err != nil {
		report.Add(env.filePath, node.StartPos().Line, node.EndPos().Line, node.StartPos().Column, node.EndPos().Column, err.Error(), report.CRITICAL_ERROR)
	}
