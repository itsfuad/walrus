...
func checkMapLiteral(node ast.MapLiteral, env *TypeEnvironment) TcValue {
	...
	for _, value := range node.Values {
		...
		if err != nil {
			errgen.Add(env.filePath, value.Key.StartPos().Line, value.Key.EndPos().Line, value.Key.StartPos().Column, value.Key.EndPos().Column, "incorrect map key. "+err.Error()).Level(errgen.NORMAL)
		}
		err = matchTypes(evaluatedMapType.(Map).ValueType, valueType)
		if err != nil {
			errgen.Add(env.filePath, value.Value.StartPos().Line, value.Value.EndPos().Line, value.Value.StartPos().Column, value.Value.EndPos().Column, "incorrect map value. "+err.Error()).Level(errgen.NORMAL)
		}
	}
