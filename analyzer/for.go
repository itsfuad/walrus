...
func checkForStmt(forStmt ast.ForStmt, env *TypeEnvironment) TcValue {
	...
	if forStmt.Init != nil || forStmt.Condition != nil || forStmt.Increment != nil {
		...
		default:
			errgen.Add(env.filePath, forStmt.StartPos().Line, forStmt.EndPos().Line, forStmt.StartPos().Column, forStmt.EndPos().Column, "for loop initialization must be a variable declaration or assignment").Level(errgen.CRITICAL)
		}
		...
		if cond == nil || cond.DType() != BOOLEAN_TYPE {
			errgen.Add(env.filePath, forStmt.StartPos().Line, forStmt.EndPos().Line, forStmt.StartPos().Column, forStmt.EndPos().Column, "for loop condition must be a boolean expression").Level(errgen.CRITICAL)
		}
		...
		if _, ok := incr.(ast.IncrementalInterface); !ok {
			errgen.Add(env.filePath, forStmt.StartPos().Line, forStmt.EndPos().Line, forStmt.StartPos().Column, forStmt.EndPos().Column, "for loop increment must be incremental assignment").Level(errgen.CRITICAL)
		}
	}
