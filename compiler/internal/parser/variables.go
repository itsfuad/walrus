...
func parseVarDeclStmt(p *Parser) ast.Node {
	...
	for {
		...
		if assignmentToken.Kind == lexer.COLON_TOKEN {
			...
			report.Add(p.FilePath, assignmentToken.Start.Line, assignmentToken.End.Line, assignmentToken.Start.Column, assignmentToken.End.Column, msg).SetLevel(report.SYNTAX_ERROR)
		}
