...
func checkForStmt(forStmt ast.ForStmt, env *TypeEnvironment) ValueTypeInterface {
	...
	if initValue != nil {
		...
		if _, ok := forStmt.Init.(ast.VarDeclStmt); !ok {
			errgen.MakeError(env.filePath, forStmt.Init.StartPos().Line, forStmt.Init.EndPos().Line, forStmt.Init.StartPos().Column, forStmt.Init.EndPos().Column, "init value must be a statement").Display()
		}
	}

	conditionValue := CheckAST(forStmt.Condition, forLoopEnv)
	if conditionValue == nil {
		errgen.MakeError(env.filePath, forStmt.Condition.StartPos().Line, forStmt.Condition.EndPos().Line, forStmt.Condition.StartPos().Column, forStmt.Condition.EndPos().Column, "condition value must be a present").Display()
	}
	...
	if incrementValue != nil {
		...
		if _, ok := forStmt.Increment.(ast.UnaryExpr); !ok {
			errgen.MakeError(env.filePath, forStmt.Increment.StartPos().Line, forStmt.Increment.EndPos().Line, forStmt.Increment.StartPos().Column, forStmt.Increment.EndPos().Column, "increment value must be a statement").Display()
		}
	}
