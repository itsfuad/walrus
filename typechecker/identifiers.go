...
func checkIdentifier(node ast.IdentifierExpr, env *TypeEnvironment) ValueTypeInterface {
	...
	if err != nil {
		errgen.MakeError(env.filePath, node.StartPos().Line, node.StartPos().Column, node.EndPos().Column, err.Error()).Display()
	}
	...
	errgen.MakeError(env.filePath, node.StartPos().Line, node.StartPos().Column, node.EndPos().Column, "failed to check type").Display()
	return nil
}