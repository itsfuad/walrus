...
func checkTypeDeclaration(node ast.TypeDeclStmt, env *TypeEnvironment) ValueTypeInterface {
	...
	default:
		...
		if err != nil {
			//errgen.AddError(env.filePath, t.StartPos().Line, t.EndPos().Line, t.StartPos().Column, t.EndPos().Column, err.Error()).DisplayWithPanic()
	...
	if err != nil {
		...
		//errgen.AddError(env.filePath, node.Start.Line, node.End.Line, node.Start.Column, node.End.Column, err.Error()).DisplayWithPanic()
